...
const EventRequests = () => {
  ...
  useEffect(() => {
    const fetchRegistrations = async () => {
      try {
        ...
        const { data } = await axios.get(`http://localhost:5000/api/registrations/event/${id}`, config);
  ...
  const handleAction = async (regId, status) => {
    try {
      ...
      await axios.patch(`http://localhost:5000/api/registrations/${regId}`, { status }, config);
